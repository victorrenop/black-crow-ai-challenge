...
class StdoutSink(Step):
    ...
    def _run_step(self, input_df: Optional[DataFrame] = None) -> DataFrame:
        input_df.show(
            n=self._row_number, truncate=self._truncated, vertical=self._vertical
        )
        return input_df
...
class JsonFileSink(Step):
    ...
    def __init__(
        ...
        save_mode: str = WriteModes.OVERWRITE,
    ...
    def _run_step(self, input_df: Optional[DataFrame] = None) -> DataFrame:
        input_df.coalesce(1).write.format("json").mode(self._save_mode).options(
            **self._write_options
